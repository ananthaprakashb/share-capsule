(()=>{
  ...
  style.textContent=`
    ...
    .contributeActions{grid-column:1/-1;display:flex;gap:10px;align-items:center;flex-wrap:wrap}.contributeSubmit{min-height:48px;border:0;border-radius:14px;padding:0 18px;background:#fff;color:#7a2e20;font-weight:950}.contributeSubmit:disabled{opacity:.6;cursor:wait}.contributeStatus{font-size:12px;font-weight:800}.contributeMail{color:#fff;font-weight:900}
  ...
  mount.innerHTML=`
    ...
    <p class="contributeIntro">Know the history behind a Tamil Nadu place name? Send the place, the naming story, and verifiable proof. The Share Capsule editorial team will review every claim before adding or correcting information on this page. Submissions are sent to <a class="contributeMail" href="mailto:editor@sharecapsule.app">editor@sharecapsule.app</a>.</p>
    ...
    <form id="placeContributionForm" class="contributeForm" novalidate>
      ...
      <div class="contributeField wide"><label for="proofLinks">Proof links</label><textarea id="proofLinks" name="proofLinks" rows="3" maxlength="2500" placeholder="One official or reputable http/https link per line."></textarea><span class="contributeHelp">Only http and https links are accepted. Links are reviewed before publication.</span></div>
      ...
      <div class="contributeField"><label for="contributorEmail">Your email</label><input id="contributorEmail" name="contributorEmail" type="email" maxlength="254" autocomplete="email"><span class="contributeHelp">Used only if the editor needs clarification.</span></div>
      ...
      <div class="contributeActions"><button class="contributeSubmit" type="submit">Send for editorial review</button><span id="contributeStatus" class="contributeStatus" role="status" aria-live="polite"></span></div>
    </form>`;
  ...
  form.addEventListener('submit',async event=>{
    ...
    try{
      const response=await fetch('/api/place-history-submissions',{method:'POST',headers:{'content-type':'application/json','accept':'application/json'},body:JSON.stringify(payload),cache:'no-store'});
      const body=await response.json().catch(()=>({}));
      if(!response.ok)throw new Error(body.error||'Unable to send submission');
      form.reset();
      status.textContent=`Received for review. Reference: ${body.reference}`;
    }catch(error){
      status.textContent=error.message||'Unable to send. Please email editor@sharecapsule.app.';
