...
const parseItems = xml => {
  ...
  }).filter(item => item.title && item.url && !seen.has(item.title.toLowerCase()) && seen.add(item.title.toLowerCase())).slice(0, 5);
};
...
export async function onRequestGet(context) {
  ...
  try {
    ...
    if (!response.ok) {
      return json({ location: locationLabel, headlines: [], message: `News source returned ${response.status}.` }, 502);
    }
    ...
    return json({ location: locationLabel, headlines: [], message: error.message || 'Local news is temporarily unavailable.' }, 502);
  }
}
