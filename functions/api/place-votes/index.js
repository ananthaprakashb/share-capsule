...
export async function onRequestGet({env,request}){
  ...
  const voteRows=await env.PLACE_VOTES_DB.prepare('SELECT place_id AS placeId, vote_count AS count FROM place_votes ORDER BY place_id').all();
  let voted=[];
  ...
  return json({votes:voteRows.results||[],voted});
}
