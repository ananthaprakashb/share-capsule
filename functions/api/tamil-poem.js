...
export async function onRequestPost(context) {
  ...
  // Retrieve body inputs from user
  const { user_prompt } = await request.json();

  const OLLAMA_BASE_URL = env.OLLAMA_BASE_URL || "https://api.groq.com/openai/v1";
  const OLLAMA_MODEL = env.OLLAMA_MODEL || "llama-3.3-70b-versatile";
  ...
  try {
    const response = await fetch(`${OLLAMA_BASE_URL}/chat/completions`, {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: "system", content: system_prompt },
          { role: "user", content: user_prompt || "இயற்கை மற்றும் மழை பற்றிய 4 வரி தமிழ் கவிதை எழுதுக" }
        ],
        temperature: 0.7
      })
    });
    ...
    if (!response.ok) {
      return new Response(JSON.stringify({ error: data.error?.message || "Inference failed" }), { status: response.status });
    }
