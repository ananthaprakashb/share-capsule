(()=>{
  ...
  async function load(){
    try{
      const [india,students,tn]=await Promise.all([json(INDIA_DATA),json(STUDENTS_DATA),json(TN_DATA)]);
      const data=scope==='tn'?tn:scope==='students'?students:india;
      const [title,subtitle]=labels[scope]||labels.tn;
      const intro=data.subtitle||subtitle;
      const childLinks=scope==='tn'||scope==='students'?'':`<div class="children"><a class="child" href="/schemes/govt/in/"><strong>India-wide schemes</strong><span>${india.schemes.length} verified central schemes →</span></a><a class="child" href="/schemes/govt/in/students/"><strong>Students</strong><span>${students.schemes.length} verified school and college benefit routes →</span></a><a class="child" href="/schemes/govt/in/tn/"><strong>Tamil Nadu</strong><span>${tn.schemes.length} verified state schemes and benefit routes →</span></a></div>`;
