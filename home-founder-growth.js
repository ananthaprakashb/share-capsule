(()=>{
  ...
  const render=data=>{
    ...
    section.innerHTML=`<p class="homeFounderGrowthEyebrow">Founder growth guide · India</p><h2>Running a small business—and ready to break through your next growth barrier?</h2><p class="homeFounderGrowthIntro">Find practical pathways for expansion capital, invoice liquidity, technology modernization, government support, industrial approvals, mentoring and corporate market access.</p><div class="homeFounderGrowthTopics"><span>💰 Capital</span><span>⚡ Working capital</span><span>🏭 Modernization</span><span>🤝 Market access</span><span>🗓 90-day roadmap</span></div><div class="homeFounderGrowthResources">${preview}</div><a class="homeFounderGrowthAction" href="${ENDPOINT}"><span>Build your business growth roadmap</span><span>Explore →</span></a><div class="homeFounderGrowthNote">Information is based on the Enterprise Growth & Scaling Resource Guide. Confirm current terms and eligibility on each linked portal.</div>`;
    toolbar.insertAdjacentElement('beforebegin',section);
  };
  const refresh=()=>fetch(`${DATA}?v=${Date.now()}`,{cache:'no-store'}).then(response=>response.ok?response.json():Promise.reject(new Error(`${DATA}: HTTP ${response.status}`))).then(data=>{window.__founderGrowthHomeData=data;render(data)}).catch(error=>console.error('Unable to show founder growth guide on home',error));
  installStyles();
  const observer=new MutationObserver(()=>window.__founderGrowthHomeData&&render(window.__founderGrowthHomeData));
  observer.observe(document.getElementById('app')||document.body,{childList:true,subtree:true});
  refresh();
  window.addEventListener('popstate',()=>setTimeout(()=>window.__founderGrowthHomeData&&render(window.__founderGrowthHomeData),0));
})();
